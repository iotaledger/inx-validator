...
func tryissueValidatorBlock(ctx context.Context) {
	// TODO: what if API changes inbetween and does not fit the block issuing time anymore?
	...
	if !deps.NodeBridge.ReadIsCommitteeMemberInEpoch(validatorAccount.ID()) {
		if !deps.NodeBridge.NodeStatus().IsBootstrapped() {
			Component.LogDebug("not issuing candidate block because node is not bootstrapped yet.")

			return
		}
		// TODO: construct and issue candidate block if not registered as validator yet
	...
	if !ParamsValidator.IgnoreBootstrapped && !deps.NodeBridge.NodeStatus().IsBootstrapped() {
...
func issueCandidateBlock(ctx context.Context, blockIssuingTime time.Time, currentAPI iotago.API) error {
	...
	if deps.NodeBridge.ReadIsCandidateInEpoch(validatorAccount.ID(), blockEpoch) {
		Component.LogDebugf("not issuing candidacy announcement block as account %s is already registered as candidate in epoch %d", validatorAccount.ID(), blockEpoch)
...
func getAddressableCommitment(ctx context.Context, blockSlot iotago.SlotIndex) (*iotago.Commitment, error) {
	protoParams := deps.NodeBridge.APIProvider().CurrentAPI().ProtocolParameters()
